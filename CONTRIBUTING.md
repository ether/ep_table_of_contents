# Contributing to ep_table_of_contents

Thanks for helping improve `ep_table_of_contents` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_table_of_contents` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_table_of_contents from your local clone
pnpm run plugins i --path ../ep_table_of_contents

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_table_of_contents/` are picked up after a server restart.
...
* Etherpad version, ep_table_of_contents version, browser + OS
* Steps to reproduce
